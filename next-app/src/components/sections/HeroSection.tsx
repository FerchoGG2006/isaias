...
export const HeroSection: React.FC = () => {
  ...
  return (
    <section id="inicio" className="relative w-full min-h-[75vh] bg-[#0C0D10] overflow-hidden text-[#F4F1EA] flex items-center justify-center py-10 sm:py-14">
      ...
      {/* 1. BACKGROUND PHOTO OR 4K VIDEO */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        {isPalacio ? (
          <video
            ...
            muted
            playsInline
            className="absolute inset-0 w-full h-full object-cover opacity-65 brightness-95 contrast-[1.08]"
          />
        ) : (
          <Image
        ...
        <div className="absolute inset-0 bg-black/60 bg-gradient-to-t from-[#0C0D10]/80 via-transparent to-[#0C0D10]/40" />

        {/* Ambient Glow */}
        <div
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[450px] rounded-full blur-[140px] ${
            isPalacio ? 'bg-gradient-to-b from-[#FF6B00]/15' : 'bg-gradient-to-b from-[#C8A96E]/10'
          }`}
        />
      </div>

      {/* 2. STANDALONE LOGO PER TENANT */}
      <div className="wrap relative z-10 w-full flex flex-col items-center justify-center text-center max-w-5xl mx-auto">
        ...
        <h1 className="sr-only">
          {isPalacio
            ? 'El Palacio de la Sublimación — Sublimación fotográfica 4K y merchandising en Valledupar'
        ...
        >
          ...
          <a
            ...
            className={`w-full sm:w-auto font-bold px-9 py-4.5 rounded-xs transition-all duration-300 shadow-2xl flex items-center justify-center gap-3 text-center shrink-0 hover:scale-105 ${
              isPalacio
                ? 'bg-[#FF6B00] hover:bg-[#ea580c] text-white shadow-[#FF6B00]/20'
                : 'bg-[#C8A96E] hover:bg-[#B8985D] text-[#0C0D10] shadow-[#C8A96E]/20'
            }`}
          >
          ...
          <Link
            href="/catalogo"
            className={`w-full sm:w-auto bg-black/60 hover:bg-black/80 border text-[#F4F1EA] font-semibold px-9 py-4.5 rounded-xs transition-all duration-300 shadow-2xl text-center shrink-0 hover:scale-105 ${
              isPalacio
                ? 'border-white/30 hover:border-[#FF6B00] hover:text-[#FF6B00]'
                : 'border-white/30 hover:border-[#C8A96E] hover:text-[#C8A96E]'
            }`}
          >
            VER CATÁLOGO
          </Link>
