...
export const AboutSection: React.FC = () => {
  const { businessId, business } = useQuote();
    ...
    <section id="taller" className="wrap py-14 sm:py-20 border-t border-white/10 scroll-mt-24">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center">
        ...
        <div className="lg:col-span-7 relative aspect-[4/3] sm:aspect-[16/11] rounded-xs overflow-hidden border border-white/10 bg-[#0C0D10] shadow-2xl group">
          <Image
            src={isPalacio ? '/assets/mug.png' : '/assets/hero-main.jpg'}
            alt={`Taller de ${business.name} en Valledupar`}
            fill
            ...
            className={`object-cover object-center group-hover:scale-105 transition-transform duration-700 ease-out brightness-90 ${
              isPalacio ? 'p-8 sm:p-14 object-contain bg-[#121318]' : ''
            }`}
          />
          ...
          <div className="absolute bottom-4 left-4 right-4 z-10 flex items-center justify-between text-xs text-[#F4F1EA] bg-black/80 backdrop-blur-md px-4 py-2.5 rounded-xs border border-white/10 font-sans">
            <span className="font-semibold text-[#C8A96E]">
              {isPalacio ? 'El Palacio de la Sublimación · Valledupar' : 'Taller en Valledupar, Cesar'}
            </span>
            <span className="text-[#A0A0A5] text-xs">
              {isPalacio ? 'Sublimación 4K & Merch' : 'Producción directa y personalizada'}
            </span>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <h2 className="font-sans font-bold text-3xl sm:text-4xl lg:text-5xl text-[#F4F1EA] tracking-tight">
              {isPalacio
                ? 'Sublimación y Merchandising en Valledupar.'
          ...
          <p className="font-sans text-sm text-[#D0CFC9] leading-relaxed font-light">
            {isPalacio
              ? 'En El Palacio de la Sublimación convertimos tus fotografías, logotipos y diseños en artículos de alta recordación: pocillos cerámicos, mugs mágicos, termos de aluminio e indumentaria deportiva con cobertura Full Print que nunca se borra.'
            ...
            {isPalacio ? (
              <>
                <div className="flex items-center gap-2.5">
                  ...
                  <span><strong>Sublimación 4K permanente:</strong> La tinta se funde a nivel molecular, sin relieve ni desgaste.</span>
                </div>
                <div className="flex items-center gap-2.5">
                  <span className="text-[#C8A96E] font-bold">✓</span>
                  <span><strong>Rígidos y textil deportivo:</strong> Mugs, termos, camisetas Qatar transpirables y gorras.</span>
                </div>
                <div className="flex items-center gap-2.5">
                  <span className="text-[#C8A96E] font-bold">✓</span>
                  <span><strong>Despachos rápidos:</strong> Desde 1 unidad hasta pedidos corporativos de cientos de piezas.</span>
                </div>
              </>
            ) : (
              <>
